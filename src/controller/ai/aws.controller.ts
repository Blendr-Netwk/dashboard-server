...
import {
  createAwsInstance,
  fetchAllAwsInstances,
  fetchInstanceStatus,
} from "@/services/aws/intances"
import {
  ...
  saveKeyPair,
  testEditFiles,
} from "@/services/prisma/instance"
import {
  fetchTransactionsByHash,
  saveTransaction,
} from "@/services/prisma/transaction"
...
class AWSController {
  ...
  public async getAllAvailableInstances(
    req: Request,
    res: Response,
    next: NextFunction
  ) {
  ...
  public async fetchAllInstanceTypes(
    req: Request,
    res: Response,
    next: NextFunction
  ) {
  ...
  public async createInstance(req: Request, res: Response, next: NextFunction) {
    try {
      ...
      const { instance, keyPair } = await createAwsInstance(
        req.user.id,
        instances.type
      )
      ...
      const savedKeyPair = await saveKeyPair(
        req.user.id,
      ...
      return res
        .status(200)
        .json({ keyPair: savedKeyPair, instance: savedInstance })
    } catch (error) {
      next(error)
      return
    }
