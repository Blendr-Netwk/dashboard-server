...
exports.getHourlyPrice = exports.fetchInstanceStatus = exports.fetchInstanceType = exports.fetchAllAwsInstances = exports.terminateAwsInstance = exports.createAwsInstance = void 0;
const instance_1 = require("../prisma/instance");
const main_1 = require("./main");
const createAwsInstance = async (userId, type) => {
    const keyPairName = `${userId}-${Date.now()}`;
    const keyPair = await main_1.awsEC2.createKeyPair({ KeyName: keyPairName }).promise();
    const params = {
    ...
    const instance = await main_1.awsEC2.runInstances(params).promise();
    return { instance, keyPair };
};
exports.createAwsInstance = createAwsInstance;
const terminateAwsInstance = async (instance) => {
    try {
        const terminatedInstance = await main_1.awsEC2.terminateInstances({ InstanceIds: [instance.instanceId] }).promise();
        if (terminatedInstance.TerminatingInstances) {
            console.log(terminatedInstance.TerminatingInstances[0]);
        }
        const deletedKeyPair = await main_1.awsEC2.deleteKeyPair({ KeyName: instance.keyName }).promise();
        (0, instance_1.updateInstance)(instance.id, { status: "terminated" });
        return true;
    }
    catch (e) {
        return false;
...
const fetchAllAwsInstances = async () => {
    let nextToken = null;
    const instances = [];
    do {
        // Filters: [{ Name: 'instance-type', Values: ['t2*', 't3*', 'g3s*', 'g4dn*', 'g4ad*', 'g5*', 'p2*', 'p3*', 'p4*'] }],
        const response = await main_1.awsEC2.describeInstanceTypes({
            NextToken: nextToken
        }).promise();
    ...
    const gpuInstanceDetails = instances
        ...
        return {
            ...
            gpu: {
                Name: instance.GpuInfo.Gpus.map((gpu) => {
...
exports.fetchAllAwsInstances = fetchAllAwsInstances;
const fetchInstanceType = async (params) => {
    return await main_1.awsEC2.describeInstanceTypes(params).promise();
};
exports.fetchInstanceType = fetchInstanceType;
const fetchInstanceStatus = async (instanceId) => {
    try {
        await main_1.awsEC2.waitFor('instanceRunning', { InstanceIds: [instanceId] }).promise();
        const updatedInstance = await main_1.awsEC2.describeInstances({ InstanceIds: [instanceId] }).promise();
        ...
        await (0, instance_1.updatePublicIpAddress)(instanceId, publicIp);
...
exports.fetchInstanceStatus = fetchInstanceStatus;
const getHourlyPrice = async (instanceType) => {
    const params = {
    ...
    try {
        const data = await main_1.awsPricing.getProducts(params).promise();
        // console.log(data);
        const priceItems = data.PriceList?.[0];
...
exports.getHourlyPrice = getHourlyPrice;
// {
//     AmiLaunchIndex: 0,
//     ImageId: 'ami-080660c9757080771',
